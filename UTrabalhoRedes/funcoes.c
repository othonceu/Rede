...
void lerRoteadores(LocalInfo *info){
	...
	while(fscanf(arq, "%d %d %s", &id, &porta, ip) != EOF){ //

	Roteador **roteadores = &info->roteadores;
...
void lerTopologia(LocalInfo *info, Topologia **vizinhos){
	...
	int id0, id1, distancia;
	if(arq == NULL){
		printf("Não foi possivel abrir o arquivo\n");
		fclose(arq);
		return;
	}
	while(fscanf(arq, "%d %d %d", &id0, &id1, &distancia) != EOF){		// Lê os dados do arquivo
		if(id0 == info->id){

			Topologia **topologia0 = vizinhos;
			Topologia *t = *topologia0;
			Topologia *novo = (Topologia *)malloc(sizeof(Topologia));
			novo->idRoteador = id1;
			...
			*topologia0 = novo;
			...
			Topologia **topologia1 = vizinhos;
			Topologia *t1 = *topologia1;
			Topologia *novo = (Topologia *)malloc(sizeof(Topologia));
			novo->idRoteador = id0;
			novo->distancia = distancia;
			novo->prox = t1;
